...
import com.phongbm.musicplayer.databinding.ActivityAlbumBinding;
...
public class AlbumActivity extends BaseActivity<ActivityAlbumBinding> implements MediaListener<Music> {
...
//        String artist =  intent.getStringExtra(MainActivity.REQUEST_ARTIST);
        String album =  intent.getStringExtra(MainActivity.REQUEST_ALBUM);


        binding.tvAlbum1.setText(album);
        arrMusic = new ArrayList<>();

        adapter = new BaseAdapter<>(this,R.layout.item_music_by_artist);

        binding.lvTest2.setAdapter(adapter);
        setDataMusic(album);
...
//        systemData.getSongByArtist()
    ...
    private void setDataMusic(String album) {
        SystemData systemData = new SystemData(this);
        ArrayList<Music> data = systemData.getSongs();
        for (Music m:data) {
            if(album.indexOf(m.getAlbum())!=-1){
                arrMusic.add(m);
            }
        }
